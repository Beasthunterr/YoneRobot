...
from AlizaRobot.modules.sql.afk_sql import is_afk, check_afk_status
...
from AlizaRobot import telethn as AlizaTelethonClient, TIGERS, DRAGONS, DEMONS
...
def get_id(update: Update, context: CallbackContext):
    ...
    if user_id:
        ...
        if chat.type == "private":
            ...
            msg.reply_text(
                f"This Group's is is <code>{chat.id}</code>.", parse_mode=ParseMode.HTML
            )
...
def info(update: Update, context: CallbackContext):
    ...
    if disaster_level_present:
        text += ' \n[<a href="https://t.me/aliza_basicinfo/3">𝐂𝐥𝐢𝐜𝐤 𝐇𝐞𝐫𝐞 𝐭𝐨 𝐤𝐧𝐨𝐰 𝐰𝐡𝐚𝐭 𝐢𝐬 𝐃𝐢𝐬𝐚𝐬𝐭𝐞𝐫 𝐋𝐞𝐯𝐞𝐥𝐬.</a>]'.format(
            bot.username
        )
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>Current stats of Aliza:</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
